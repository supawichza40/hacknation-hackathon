...
- **T-4h (05:00 ET), in order:** R7 scan-replay beat (seeded cards remain) → R1 query box → axis trends (static scores) → R8 guided tour → showcase card #2 → graph filters/domains → outreach draft → Founder-Score sparkline → **(absolute last resort) the graph-grounded chat trio**. R2 chip is too small to ladder. Then STOP building: video, README, push, submit.
