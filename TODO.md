...
| **Last push** | **LIVE DEMO UP** @ 2026-07-19 15:45 BST — https://foundergraph.onrender.com (login `investor@foundergraph.demo` / `demo`). Root cause of every silent Render build death: uncommitted typescript@5.9.3 downgrade (Render installed typescript@7 → Next TS-config-validation worker silent exit 1). Fixed in `cd59b9f` (+ `1f97e12` turbopack:{} from parallel session, rolling). Browser-verified live: login → Pipeline (3 cards, scores, thesis fit) → Lattice-DB diligence (score 74, voice-brief button, 40-node graph). Local fallback also serving on localhost:3000. Each deploy/restart reseeds demo data. |
